...
void GameOver::show()
{
    ...
    if (ranking[0].first != -1 && !tmp_ranking->empty())
    {
        if (m_winners.size() > 2 && ranking[0].second != ranking[1].second)
        {
            ...
            st_text = env->addStaticText(ss.str().c_str(),
        ...
        st_text = env->addStaticText(L"Draw !",
                                     irr::core::rect<irr::s32>(0, 100, (irr::s32) IrrlichtController::width, 200),
                                     false, true);
    st_text->setOverrideColor(irr::video::SColor(255, 255, 255, 255));
    st_text->setTextAlignment(irr::gui::EGUI_ALIGNMENT::EGUIA_CENTER, irr::gui::EGUI_ALIGNMENT::EGUIA_CENTER);
    env->drawAll();
}
